  ...
  async function generatePDF() {
    ...
    try {
      ...
      for (let i = 0; i < allFormSteps.length; i++) {
        ...
        const imgHeightInPdf =
          (imgProps.height * usablePdfWidth) / imgProps.width;

        if (
          currentYPositionInPdf !== margin &&
          currentYPositionInPdf + imgHeightInPdf >
