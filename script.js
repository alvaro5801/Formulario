...
document.addEventListener('DOMContentLoaded', function () {
            ...
            if (canvas.width !== newWidth || canvas.height !== newHeight) {
                ...
                if (canvas.width > 0 && canvas.height > 0 && !isCanvasBlank(ctx, canvas.width, canvas.height)) {
                    try {
                        ...
                        console.warn("Could not get image data before resize for " + canvasId, e)
                    }
                }
    ...
    async function generatePDF() {
        ...
        // Garante que todos os passos estejam visíveis para a captura
        allFormSteps.forEach(step => step.style.display = 'block');
        ...
        try {
            ...
            // Largura de captura consistente (simulando um desktop)
            const captureWidth = 900; 
            
            // Salva a largura original e aplica a largura de captura
            const originalWidth = formContainerForPdf.style.width;
            formContainerForPdf.style.width = captureWidth + 'px';

            await new Promise(resolve => setTimeout(resolve, 100)); // Pequeno delay para o DOM atualizar

            // Captura o contêiner inteiro de uma vez
            const canvas = await html2canvas(formContainerForPdf, {
                scale: 2,
                useCORS: true,
                logging: false,
                width: captureWidth, // Usa a largura de captura
                windowWidth: captureWidth,
            });

            // Restaura a largura original
            formContainerForPdf.style.width = originalWidth;

            const imgData = canvas.toDataURL('image/png');
            const imgProps = pdf.getImageProperties(imgData);
            const imgHeight = (imgProps.height * usablePdfWidth) / imgProps.width;

            // Adiciona a imagem ao PDF (com quebra de página se necessário)
            let heightLeft = imgHeight;
            let position = margin;

            pdf.addImage(imgData, 'PNG', margin, position, usablePdfWidth, imgHeight);
            ...
            // Limpa o estado de impressão
            formContainerForPdf.classList.remove('printing-pdf');
            submitButton.innerHTML = originalButtonText;
            submitButton.disabled = false;
            showStep(currentStep); // Volta para a etapa atual
        }
    }
    ...
    function saveFormState() {
        ...
        if (localDataSignaturePad && !localDataSignaturePad.isEmpty()) {
            try {
                formData['localDataCanvas'] = document.getElementById('localDataCanvas').toDataURL();
            } catch (e) {
        ...
        if (assinaturaSignaturePad && !assinaturaSignaturePad.isEmpty()) {
            try {
                formData['assinaturaCanvas'] = document.getElementById('assinaturaCanvas').toDataURL();
            } catch (e) {
